...
export interface INotification extends Document {
  user_id: string;
  type: 'post_approved' | 'post_rejected' | 'revision_requested' | 'article_approved' | 'article_rejected' | 'post_edited' | 'article_edited';
  post_id: string;
  post_title: string;
  message: string;
  read: boolean;
...
const notificationSchema = new Schema<INotification>(
  {
    ...
    type: { type: String, required: true, enum: ['post_approved', 'post_rejected', 'revision_requested', 'article_approved', 'article_rejected', 'post_edited', 'article_edited'] },
