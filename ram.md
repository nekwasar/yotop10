...
### Public loading skeletons + light-for-all + super-admin rotation (2026-09-15, [M26.1]–[M26.2])
- **Skeletons (public only, admin excluded per scope)**: new `SearchSkeleton`, `NotificationDetailSkeleton`, `HistorySkeleton` (shared by post + username history) in `frontend/src/components/`, matching existing skeleton conventions (`animate-pulse` + `bg-white/*`, all light-mode covered). Wired into `search/client` (initial results), `notifications/[id]/client`, `[slug]/history/client`, `username-history/client`.
