...
- **Article edit (new)**: backend `GET /admin/articles/:id` (any status) + `PATCH /admin/articles/:id` (title/body/category/cover/sources, no version lock — Article has no version field), frontend `admin/articles/[id]/edit/` page, `admin/articles` all-list page (status filter/search/stats/bulk-approve/Edit buttons), Edit button on pending-detail.
