/**
 * @[username]/page.tsx — Public user profile page.
 *
 * • Server-side rendered (SSR) for SEO + OG social-card support.
 * • Fetches data from the FastAPI backend at build/request time.
 * • Renders different content depending on profile privacy (full / limited / 404).
 * • Author tier triggers gold visual treatment.
 */
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
    ...
    if (!profile) {
        return { title: "Profile not found — YoTop10" };
    }
    return {
        title: `${profile.display_name} (@${profile.username}) — YoTop10`,
        description: profile.bio ?? `Check out ${profile.display_name}'s debating profile on YoTop10.`,
        openGraph: {
...
export default async function UserProfilePage({ params }: Params) {
    ...
    // 404 — private profile or non-existent user
    if (!profile) notFound();

    // Restricted / connections-only view
        ...
        <div className="max-w-3xl mx-auto py-8 px-2">
            ...
            >
                ...
                <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                    {/* Avatar */}
                    <div
                        className={`shrink-0 w-24 h-24 rounded-full overflow-hidden shadow-lg ${isAuthor
                                ? "ring-4 ring-amber-400/70 shadow-[0_0_20px_rgba(245,158,11,0.4)]"
                                : "ring-2 ring-(--border-accent)"
                            }`}
                    >
                        {profile.avatar_url ? (
                            <img
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                        ...
                        {profile.bio && (
                            <p className="text-[var(--text-primary)] text-sm leading-relaxed mt-1 max-w-lg">
                                {profile.bio}
                            </p>
                        )}
                    </div>
                ...
                {/* Action buttons — wired in M10 */}
                <div className="mt-6 flex gap-3 flex-wrap">
                    <button
                        type="button"
                        disabled
                        className="px-6 py-2.5 rounded-full font-mono font-bold text-sm tracking-widest uppercase bg-[var(--brand-primary)]/10 border border-[var(--brand-primary)]/40 text-[var(--brand-primary)] opacity-60 cursor-not-allowed"
                    >
                        Follow
                    </button>
                    <button
                        type="button"
                        disabled
                        className="px-6 py-2.5 rounded-full font-mono font-bold text-sm tracking-widest uppercase bg-[var(--bg-surface)] border border-(--border-accent) text-[var(--text-muted)] opacity-60 cursor-not-allowed"
                    >
                        Connect
                    </button>
                </div>
