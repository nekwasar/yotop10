                ...
                {r._type === 'post' ? (
                  <Link href={`/${r.slug}`} className="block group">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-600">
                      <span>By {toPublicSlug(r.author_display_name || r.author_username)}</span>
                      {r.category_slug && (
                  ...
                  <Link href={`/${r.post_slug || ''}`} className="block group">
                    ...
                    <div className="text-xs text-zinc-600">
                      By {toPublicSlug(r.author_display_name || r.author_username)} &middot; <span suppressHydrationWarning>{formatDate(r.created_at)}</span>
                    </div>
                  </Link>
                )}
              </div>
